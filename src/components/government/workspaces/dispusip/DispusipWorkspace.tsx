  ...
  const handleReject = async (reason: string) => {
    ...
    try {
      await updateDoc(doc(db, COLLECTIONS.ORDERS, orderId), {
        ...
        rejectedByDinasName: userData?.displayName || "Petugas Dispusip",
      ...
      if (user) {
        await writeAuditLog({
          ...
          action: "rejected",
          actorId: user.uid,
          actorName: userData?.displayName || "Petugas Dispusip",
  ...
  const handleApproveBookOrder = async (orderId: string) => {
    ...
    try {
      await updateDoc(doc(db, COLLECTIONS.ORDERS, orderId), {
        ...
        verifiedByDinasName: userData?.displayName || "Petugas Dispusip",
        updatedAt: serverTimestamp()
      });
