...
export function DiskopLegalitasNibForm({ agency, service, onSuccess, onCancel }: CivicSubServiceFormProps) {
  ...
  const [businessSector, setBusinessSector] = useState("Kuliner Tradisional Solo");
  const [omzetBulananEstimasi, setOmzetBulananEstimasi] = useState("");
    ...
    const orderId = await submitOrder(
      {
        ...
        price: 0, // Pendampingan OSS Gratis
        pickupLocation: {
          address: "Pusat Layanan Usaha Terpadu (PLUT) Solo",
          lat: -7.5695,
          lng: 110.8285
        },
        dropoffLocation: {
    ...
    <form onSubmit={handleSubmit} className="space-y-3.5">
      <div className="space-y-2.5 p-3.5 rounded-2xl bg-emerald-50/60 dark:bg-emerald-950/20 border border-emerald-200/70 dark:border-emerald-900/40">
        <div className="flex items-center gap-2 text-emerald-700 dark:text-emerald-400 pb-1 border-b border-emerald-200/60 dark:border-emerald-900/40">
          ...
          <span className="text-xs font-bold">Fasilitasi Sertifikasi Legalitas & NIB OSS RBA UMKM</span>
        </div>

        <CivicTextField
          label="Nama Usaha Mikro / Brand Warung"
          value={businessName}
          onChange={setBusinessName}
          placeholder="Contoh: Warung Soto Mbok Nem..."
          required
        />

        <CivicSelectField
          label="Sektor Usaha UMKM"
          value={businessSector}
          onChange={setBusinessSector}
          options={[
            "Kuliner Tradisional Solo",
            "Batik & Kriya Seni Triwindu",
            "Toko Kelontong & Sembako",
            "Jasa Kreatif & Percetakan",
            "Pertanian & Pengolahan Pangan"
          ]}
        />
        
        <CivicTextField
          label="Estimasi Omzet Bulanan (Opsional)"
          type="number"
          value={omzetBulananEstimasi}
          onChange={setOmzetBulananEstimasi}
      ...
      <CivicPriceFooter
        label="Biaya Pendampingan NIB:"
        sublabel="100% Gratis Program PLUT Diskop UKM Surakarta"
        priceText="GRATIS (Program PLUT)"
      ...
      <CivicSubmitButton
        ...
        submitText="Ajukan Pendampingan NIB OSS"
        onCancel={onCancel}
