...
import { Baby, User, Phone, MapPin, Building } from "lucide-react";

export function DukcapilKiaAkteForm({ agency, service, onSuccess, onCancel }: CivicSubServiceFormProps) {
  ...
  const [docType, setDocType] = useState("Kartu Identitas Anak (KIA Usia 0-17)");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const orderId = await submitOrder(
      {
        customerId: user.uid,
        customerName: parentName || userData?.displayName || "Warga Surakarta",
        customerPhone: phone,
    ...
    <form onSubmit={handleSubmit} className="space-y-3.5">
      <div className="space-y-2.5 p-3.5 rounded-2xl bg-blue-50/60 dark:bg-blue-950/20 border border-blue-200/70 dark:border-blue-900/40">
        <div className="flex items-center gap-2 text-blue-700 dark:text-blue-400 pb-1 border-b border-blue-200/60 dark:border-blue-900/40">
          ...
          <span className="text-xs font-bold">Layanan Antar KIA & Akta Catatan Sipil Anak</span>
        </div>

        <CivicSelectField
          label="Pilihan Dokumen Anak / Catatan Sipil"
          value={docType}
          onChange={setDocType}
          options={[
            "Kartu Identitas Anak (KIA Usia 0-17)",
        ...
        <CivicTextField
          ...
          placeholder="Nama anak sesuai akta / KK..."
          required
        />

        <CivicSelectField
          label="Loket Pengambilan Dokumen"
      ...
      <CivicAddressSelector
        label="Alamat Pengantaran KIA / Akta"
        value={address}
      ...
      <CivicSubmitButton
        ...
        submitText="Ajukan Pengantaran KIA / Akta"
        onCancel={onCancel}
