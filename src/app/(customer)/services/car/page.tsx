  ...
  const calculateRoute = async () => {
    ...
    try {
      if (typeof window !== "undefined" && window.google?.maps) {
        ...
        setPrice(Math.max(15000, Math.round(estDist * 4500)));
      }
    } catch (err) {
          ...
          <div className="relative z-30 max-w-lg w-full mx-auto px-4 pt-4 pointer-events-none">
            <div className="flex items-center justify-between pointer-events-auto">
              ...
              <div className={`px-3.5 py-1.5 rounded-full text-white text-xs font-bold shadow-2xl backdrop-blur-xl flex items-center gap-1.5 ${
                manualPickType === "pickup" ? "bg-teal-600" : "bg-rose-600"
              }`}>
                {manualPickType === "pickup" ? <MapPin className="h-3.5 w-3.5" /> : <Navigation className="h-3.5 w-3.5" />}
                <span>Geser Peta ke Titik {manualPickType === "pickup" ? "Jemput Mobil" : "Tujuan Mobil"}</span>
              </div>
            </div>
          </div>
          ...
          <div className="relative z-30 max-w-lg w-full mx-auto px-4 pb-6 mt-auto">
            ...
            >
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">
                    Titik {manualPickType === "pickup" ? "Penjemputan Mobil" : "Tujuan Mobil"} Terpilih:
                  </span>
                  {isResolvingAddress && (
                ...
                <div className="flex items-start gap-2.5">
                  <div className={`p-2 rounded-xl mt-0.5 shrink-0 ${
                    manualPickType === "pickup" ? "bg-teal-500/15 text-teal-600 dark:text-teal-400" : "bg-rose-500/15 text-rose-600 dark:text-rose-400"
                  }`}>
              ...
              >
                ...
                <span>Pasang Sebagai Titik {manualPickType === "pickup" ? "Jemput Mobil" : "Tujuan Mobil"}</span>
              </Button>
            </motion.div>
          </div>
        </>
          ...
          <div className="relative z-20 max-w-lg w-full mx-auto px-3.5 pt-3 space-y-2 pointer-events-none">
            ...
            <div className="flex items-center justify-between pointer-events-auto">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <div className="bg-white/95 dark:bg-[#0c1220]/95 border border-slate-200/80 dark:border-white/[0.08] px-3 py-1 rounded-full backdrop-blur-md text-[11px] font-bold text-teal-600 dark:text-teal-400 flex items-center gap-1.5 shadow-md">
                  <span className="w-1.5 h-1.5 rounded-full bg-teal-500 animate-pulse" />
                  <Car className="h-3.5 w-3.5 text-teal-500" /> Mobil Solo
                </div>
              </div>
            </div>
              ...
              >
                <div className="flex-1 min-w-0 space-y-0.5">
                  ...
                  <p className="text-[10px] text-slate-500 dark:text-zinc-400">
                    Jarak Tempuh: <strong className="text-teal-600 dark:text-teal-400">{distanceKm.toFixed(1)} KM</strong>
                  </p>
                </div>
            ...
            <div className="bg-white/95 dark:bg-[#0c1220]/95 border border-slate-200/80 dark:border-white/[0.08] rounded-2xl shadow-2xl p-3.5 backdrop-blur-2xl space-y-2.5">
              {price === 0 ? (
                <Button 
                  ...
                  className="w-full h-11 bg-teal-600 hover:bg-teal-500 text-white font-bold rounded-xl text-xs flex items-center justify-center gap-2 cursor-pointer shadow-lg transition-all"
                >
                  {isCalculatingPrice ? <Loader2 className="h-4 w-4 animate-spin text-white" /> : <Sparkles className="h-4 w-4 text-teal-200" />}
                  {isCalculatingPrice ? "Menghitung Rute..." : "Hitung Jarak & Tarif Rute"}
                </Button>
              ) : (
                <div className="space-y-2.5">
                  <div className="flex items-center justify-between p-2.5 bg-teal-500/10 dark:bg-teal-500/15 rounded-xl border border-teal-500/20">
                    <div>
                      <div className="flex items-center gap-1.5">
                        <span className="text-xs font-bold text-slate-900 dark:text-white">Tarif Mobil Bersih</span>
                        <span className="text-[9px] bg-teal-500/20 text-teal-600 dark:text-teal-400 px-1.5 py-0.2 rounded font-bold">
              ...
              <Button 
                className="w-full h-12 bg-gradient-to-r from-teal-600 to-cyan-600 hover:from-teal-500 hover:to-cyan-500 text-white font-black rounded-xl shadow-lg shadow-teal-600/30 text-xs cursor-pointer flex items-center justify-center gap-2"
                onClick={handleOrder}
                disabled={isOrdering}
              >
                {isOrdering ? (
